...
const agreementFor = (values: MndaValues = sampleValues()): Agreement =>
...
describe("AgreementDocument", () => {
  ...
  it("renders a rule to write on where a value is missing", () => {
    const { container } = render(
      <AgreementDocument agreement={agreementFor(sampleValues({ governingLaw: "" }))} />,
    );
  ...
  it("shows no rules when every value is supplied", () => {
    const { container } = render(<AgreementDocument agreement={agreementFor()} />);
  ...
  describe("signature table", () => {
    ...
    it("puts each party's details in its own column", () => {
      render(<AgreementDocument agreement={agreementFor()} />);
      const row = screen.getByRole("rowheader", { name: "Print Name" }).closest("tr")!;
    ...
    it("keeps a multi-line notice address on separate lines", () => {
      render(<AgreementDocument agreement={agreementFor()} />);
      const row = screen.getByRole("rowheader", { name: "Notice Address" }).closest("tr")!;
      const cells = within(row).getAllByRole("cell");
  ...
  describe("standard terms", () => {
    ...
    it("renders links with their target", () => {
      ...
      expect(licence).toHaveAttribute("href", expect.stringContaining("creativecommons.org"));
    });

    it("carries the Common Paper attribution required by the licence", () => {
      render(<AgreementDocument agreement={agreementFor()} />);
      expect(screen.getAllByText(/free to use under/).length).toBeGreaterThan(0);
    });
  });

  it("updates when the values change", () => {
    const { rerender } = render(<AgreementDocument agreement={agreementFor()} />);
    expect(screen.getByText("Expires 2 years from Effective Date.")).toBeInTheDocument();

    rerender(
      <AgreementDocument
        agreement={agreementFor(sampleValues({ mndaTermKind: "untilTerminated" }))}
      />,
    );
    expect(
